...
var current = Parse(input.First());
foreach (var line in input.Skip(1))
{
    // Console.WriteLine("{0}", line);
    // Console.WriteLine("{0}", Parse(line));
    // Console.WriteLine();

    current = new SnailPair(current, Parse(line));
    PopulateParents(current, null);
    current = Reduce(current);
}

Console.WriteLine("{0}", current.Magnitude);
